...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_2`` of
``lens/config.py``, which is the ensemble with selection driven by the referee
estimate alone and self-training switched off. It is not a port of a published
method and it should not be read as one; the row it produces says what this
mechanism is worth inside this harness.

Estimating member competence with a meta-learner is the idea behind
meta-learning approaches to dynamic ensemble selection. That is the motivation
for the cell; the referee here is this repository's own multi-target model, not
any published meta-feature pipeline, and no published method is reproduced.
...
# -------------------------------------------------------------------- mechanism
class RefereeAccuracySelection(LENS):
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=RefereeAccuracySelection,
