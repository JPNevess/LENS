"""Adaptive Random Forest, the unweighted baseline of the comparison.

A bagged forest of Hoeffding trees over random feature subspaces, with drift
detection per member. Every member votes and every vote counts the same: there
is no dynamic selection and no use of unlabelled instances. It is the floor the
other columns are measured against, because it isolates what the ensemble
achieves from supervision alone.

Mechanism
---------
Members are Hoeffding Adaptive Trees trained with random patches, that is
feature subsampling combined with Poisson bagging. A background pool replaces
members that decay after a drift. Nothing else is switched on.
...
--------------------------------
The published ARF uses its own drift and warning detectors per tree. Here the
members are Hoeffding Adaptive Trees, which carry their own internal drift
adaptation, and the ensemble-level pool replacement plays the role of the
background-tree mechanism.
...
Reference: Gomes et al., Adaptive random forests for evolving data stream classification, Machine Learning 106(9), 2017
"""
import argparse
import os
import sys
...
from lens.config import (DIVERSITY_DISAGREEMENT, CONFIG_BASE,
                         INF_NONE, TR_NONE)
...
# -------------------------------------------------------------------- mechanism
class AdaptiveRandomForest(LENS):
    """Uniform majority vote: every member is equally competent."""

    def competence(self, margins, est_acc):
        """Constant competence, so no member is preferred and no member is dropped."""
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=AdaptiveRandomForest,
