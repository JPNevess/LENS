"""Self-training gated by the member's own margin.

A member trains on its own prediction for an unlabelled instance whenever that
prediction is confident enough. The vote is uniform, so this cell differs from
the uniform-vote floor only in what it does with unlabelled data.

The weakness of the mechanism is built into it: a member that is confidently
wrong will reinforce its own error, and nothing in the loop can contradict it.
The cells that gate on the referee or on the disagreement of other members
exist to isolate exactly that difference.
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_5`` of
``lens/config.py``, which is the ensemble with selection switched off and self-
training gated by the member's own margin. It is not a port of a published
method and it should not be read as one; the row it produces says what this
mechanism is worth inside this harness.

Training a learner on its own confident predictions is the classical self-
training loop. The cell isolates that loop inside this harness; it is not a
reproduction of any particular published self-training algorithm.
...
# -------------------------------------------------------------------- mechanism
class SelfTrainMargin(LENS):
    """Uniform vote; this cell differs from the floor only in how it trains."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- self-training gated by the member's own margin",
