...
Training is learning by disagreement with a confident electorate. Only members
whose margin clears the threshold vote on the pseudo-label, and the weight is
the geometric mean of the member's running estimated accuracy and the agreeing
fraction among those voters. Requiring both a reliable member and a strong
consensus is what keeps the pseudo-labels clean.

Adaptation: ADWIN monitors a meta-learned signal; on drift lambda shifts
towards diversity and weak members are replaced from the background pool.
...
from lens.config import (DIVERSITY_DISAGREEMENT, CONFIG_12,
                         INF_MMR, TR_TRAIN_W)
