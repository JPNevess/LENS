"""Selection by the product of estimated accuracy and margin.

The two competence signals are combined: the referee's estimate that a member
is correct here, and how decided that member is here. The geometric mean
requires both, so a member that is reliable but hesitant, or decided but
unreliable, ranks below one that is neither.

This is the relevance score that the full method later feeds into maximal
marginal relevance; on its own it is a pure ranking, with no diversity term.

Selection at prediction time
    members ranked by sqrt(A-hat * M); the top-K vote
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_3`` of
``lens/config.py``, which is the ensemble with the combined competence score
driving selection and self-training switched off. It is not a port of a
...
Ranking members by a local competence estimate and letting the most competent
ones decide is the common structure of the dynamic ensemble selection
literature. The cell exists to isolate that structure; the score is this
repository's own and no published selection method is reproduced.
...
from lens.config import (DIVERSITY_DISAGREEMENT, CONFIG_3,
                         INF_SQRTAM, TR_NONE)
...
# -------------------------------------------------------------------- mechanism
class CompetenceSelection(LENS):
    """Competence is the geometric mean of estimated accuracy and margin."""

    def competence(self, margins, est_acc):
        """Geometric mean of the two signals, so a member has to be both reliable and decided to rank high."""
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=CompetenceSelection,
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- selection by sqrt(estimated accuracy * margin)",
