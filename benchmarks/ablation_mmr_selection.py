"""Selection by maximal marginal relevance: competence against redundancy.

Members are picked greedily, each one scored by its competence minus its
similarity to the members already chosen. The trade-off is controlled by
lambda, which shifts towards diversity after a drift is detected.

Ranking by competence alone tends to select members that agree with each other,
because agreement is what makes them all look competent. Discounting redundancy
is what stops the selected subset from collapsing onto one opinion.
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_4`` of
``lens/config.py``, which is the ensemble with maximal marginal relevance
selection switched on and self-training switched off. It is not a port of a
published method and it should not be read as one; the row it produces says
what this mechanism is worth inside this harness.

Trading competence against diversity when composing a stream ensemble is an
idea this cell shares with the ensemble-diversification literature. That is why
it is in the grid; the selection here uses this repository's own competence and
diversity estimates, and no published diversification method is reproduced.
...
from lens.config import (DIVERSITY_DISAGREEMENT, CONFIG_4,
                         INF_MMR, TR_NONE)
...
# -------------------------------------------------------------------- mechanism
class MMRSelection(LENS):
    """Relevance is the geometric mean of estimated accuracy and margin; the diversity term enters through the maximal marginal relevance step."""

    def competence(self, margins, est_acc):
        """Relevance for the maximal marginal relevance step, before the redundancy discount is applied."""
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- selection by maximal marginal relevance",
