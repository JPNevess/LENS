...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_6`` of
``lens/config.py``, which is the ensemble with selection switched off and self-
training driven by disagreement with the other members. It is not a port of a
...
Letting learners teach each other from disagreement, over different views of
the data, is the co-training idea. The cell isolates it in a form adapted to a
single forest -- many random subspaces instead of two independent views -- and
does not reproduce any published co-training algorithm.
...
# -------------------------------------------------------------------- mechanism
class DisagreementTraining(LENS):
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=DisagreementTraining,
