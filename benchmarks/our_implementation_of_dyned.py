...
Ranking members by competence alone tends to select members that agree, which
wastes the ensemble. Maximal marginal relevance discounts a candidate by how
redundant it is with the members already chosen, so the selected subset is
accurate and complementary rather than merely confident.

Mechanism
---------
Competence is the same geometric mean used by dynamic ensemble selection. What
DynED adds is the redundancy term: members are picked greedily, each one scored
by its relevance minus its similarity to those already picked, with lambda
setting the balance. Lambda shifts towards diversity when a drift is detected.

Unlabelled instances are discarded, which is deliberate: it isolates the
selection mechanism from any semi-supervised effect.
...
relevance over a measured competence. Here the competence is estimated by the
referee rather than measured against labels, which is what allows the method to
run when almost no labels arrive.
...
# -------------------------------------------------------------------- mechanism
class DynamicEnsembleDiversification(LENS):
    """Competence is the geometric mean of estimated accuracy and margin; the diversity term enters through the maximal marginal relevance selection."""
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=DynamicEnsembleDiversification,
