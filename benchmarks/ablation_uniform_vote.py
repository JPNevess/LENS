"""Uniform majority vote: no selection, no self-training.

The floor of the ablation. Every member votes with equal weight and only
labelled instances are used for training, so neither of the two axes is
switched on. Every other row in the table is this configuration plus one
mechanism, which is what makes it the reference point.
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_BASE`` of
``lens/config.py``, which is the ensemble with both the selection and the self-
training axis switched off. It is not a port of a published method and it
should not be read as one; the row it produces says what this mechanism is
worth inside this harness.
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- uniform vote, no selection, no self-training",
