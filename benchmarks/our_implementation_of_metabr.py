"""Dynamic selection driven by a meta-learner, in the spirit of META-DES.

A meta-learner is trained to predict, per instance, whether each member is
currently correct. Members are then ranked by that estimate rather than by
their own confidence. This is the meta-learning route to dynamic ensemble
selection, transported to a setting where true labels are mostly missing.

Mechanism
---------
The referee is a multi-target model: one target per ensemble member, each
target being whether that member is right. It is updated only on the labelled
instances, and queried on every instance.

Selection at prediction time
    members ranked by the referee's estimate A-hat; the top-K vote
...
--------------------------------
META-DES selects competent members from a validation neighbourhood of the query
using several meta-features. Here the neighbourhood and the meta-features are
replaced by an online meta-learner that generalises across the stream, because
a stored validation set is not available under concept drift.
