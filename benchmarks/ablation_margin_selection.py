"""Selection by prediction margin.

Members are ranked by how decided they are on the instance at hand: the margin
between the top two class probabilities. Confident members carry the vote and
hesitant ones are weighted down.

The margin needs no label, which is what makes it usable in the sparsely
labelled setting. It is a purely local signal: it says how sure a member is
here, not how often it is right.

Selection at prediction time
    members ranked by margin M; the top-K vote, weighted by margin
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_1`` of
``lens/config.py``, which is the ensemble with selection driven by the margin
alone and self-training switched off. It is not a port of a published method
and it should not be read as one; the row it produces says what this mechanism
is worth inside this harness.

Confidence-based weighting of this kind is the signal behind abstention and
selective-classification ensembles, where a low margin triggers a rejection
instead of a down-weighting. That literature is why the cell is in the grid; no
abstention method is implemented here, and nothing here abstains.
...
from lens.config import (DIVERSITY_DISAGREEMENT, CONFIG_1,
                         INF_MARGIN, TR_NONE)
...
# -------------------------------------------------------------------- mechanism
class MarginSelection(LENS):
    """Competence is the member's prediction margin on the current instance."""

    def competence(self, margins, est_acc):
        """Margin between the two most likely classes, so a member that is hesitating here counts for less."""
        return margins
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- selection by prediction margin",
