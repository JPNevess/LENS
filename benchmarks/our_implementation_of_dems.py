"""Dynamic ensemble selection combining estimated accuracy with local confidence.

Neither signal is sufficient alone: the referee's estimate says how reliable a
member has been, the margin says how decided it is here. Their geometric mean
requires both to be high, so a member that is usually good but hesitating now,
or decided now but generally unreliable, is ranked below one that is both.

Mechanism
---------
The geometric mean is used rather than the arithmetic one precisely because it
punishes imbalance: it collapses towards the smaller of the two factors.
...
--------------------------------
Classical DES estimates local competence in a neighbourhood of the query, which
assumes a stored and still-valid reference set. Here the estimate comes from
the online referee, and the local part of the signal is the margin.
...
Reference: Ko et al., From dynamic classifier selection to dynamic ensemble selection, Pattern Recognition 41(5), 2008
"""
import argparse
import os
import sys
...
# -------------------------------------------------------------------- mechanism
class DynamicEnsembleSelection(LENS):
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=DynamicEnsembleSelection,
