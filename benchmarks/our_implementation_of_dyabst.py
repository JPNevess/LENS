"""Dynamic selection by prediction margin, in the spirit of abstention-based ensembles.

Members are ranked by how decided they are on the instance at hand: the margin
between the top two class probabilities. The confident members carry the vote
and the hesitant ones are effectively set aside, which is the abstention idea
applied per member rather than per prediction.

Mechanism
---------
The margin is available without any label, which is what makes this usable in
the sparsely labelled setting. It is a purely local signal: it says how sure a
member is here, not how often it is right.
...
--------------------------------
Abstention methods usually reject the instance when confidence is low. Here the
same confidence signal is used to weight members instead of to reject, so the
ensemble always answers and the comparison stays on accuracy rather than on
coverage.
...
Reference: Hullermeier et al., dynamic abstention and selective classification in ensembles
"""
import argparse
import os
import sys
...
def evaluate(path, label_pct, seed, max_instances=0):
    ...
    return run_experiment(
        ...
        ensemble_cls=DynamicAbstention,
