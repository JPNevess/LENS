"""Self-training gated and weighted by the referee.

As in the margin-gated cell, a member trains on its own prediction for an
unlabelled instance, but the gate and the weight come from the referee's
estimate that this member is correct here rather than from the member's own
confidence.

The pair of cells isolates one question: whether a learned, external estimate
of correctness admits better pseudo-labels than the learner's own confidence.
The vote is uniform in both, so nothing else differs.
...
What this file is
-----------------
One cell of the factorial ablation in this repository: ``CONFIG_13`` of
``lens/config.py``, which is the ensemble with selection switched off and self-
training gated by the referee estimate. It is not a port of a published method
and it should not be read as one; the row it produces says what this mechanism
is worth inside this harness.

Self-labelling under a learned confidence criterion is a family of techniques
in the semi-supervised literature. The cell isolates that idea; it is not a
port of any of them.
...
def build_parser():
    p = argparse.ArgumentParser(
        description=f"{NAME} -- self-training gated by the referee estimate",
