...
Provenance
----------
The drift geometry here is not the one the committed CSVs under ``results/``
were produced from: transitions are wider, abrupt change points are irregularly
spaced rather than evenly spaced, and the seeds differ. Streams generated by
this module will therefore not reproduce those numbers. See the note in
``README.md``.
"""
import os
