"""Generate the AGR_g stream.

The same Agrawal concepts, joined by gradual transitions.

Geometry
--------
Identical to AGR_a except that the transitions are spread over
``GRADUAL_WIDTH`` instances around the centres in
``lens/streams.GRADUAL_POS``.
...
comparable with another set generated from the same values.
    ...
    python data/make_agr_g.py                  # writes data/AGR_g.arff
    python data/make_agr_g.py --instances 50000
    python data/make_agr_g.py --seed 1234 --out /tmp/AGR_g.arff
...
def main(argv=None):
    ...
    p.add_argument("--out", default=None,
                   help="output path (default: data/AGR_g.arff)")
